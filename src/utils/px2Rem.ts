...
class Px2Rem {
    ...
    setRem = (): void => {
        ...
        // 获取真正的宽度
        const realCwidth = document.body.clientWidth || document.documentElement.clientHeight;
        this.cWidth = realCwidth > 1285 ? 1285 : realCwidth < 800 ? 800 : realCwidth;
        //进行换算，规则为 真正的宽度/草稿纸的宽度 * 倍数 = 1rem
        html.style.fontSize = (this.cWidth / this.pWidth) * this.pRem + 'px';
    };
    getRemToPx = (rem: number) => {
